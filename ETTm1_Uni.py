...
input_window = 1200           # input_window = previous time steps - output_window
output_window = 672
start_lr = 0.00001
epochs = 35
...
train_data, val_data, test_data = get_ETTm1_data(dataset=dataset, target=target, input_window=input_window, output_window=output_window, device=device)

if with_Unet == True:
    model = Transformer_ETTm1_UNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding).to(device)
else:
    model = Transformer_ETTm1_noUNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding).to(device)
