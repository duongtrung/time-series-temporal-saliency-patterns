...
data = "ETTh2-AF"                # Options: "ETT", "Electricity", "weather", "ETT-N-HITS"
dataset = "ETTh2.csv"       # Options: "ETTh1.csv", "ETTh2.csv", "ETTm1.csv", "ECL.csv", "WTH.csv" 
...
input_window = 840           # input_window = previous time steps - output_window
output_window = 720
start_lr = 0.00005
epochs = 25
...
train_data, val_data, test_data = get_ETTh2_AF_mul(dataset=dataset, target=target, input_window=input_window, output_window=output_window, device=device, number_of_cols=number_of_cols)

if with_Unet == True: 
    model = v5_ETTh2_Mul_UNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding, number_of_cols=number_of_cols).to(device)
else:
    model = v5_ETTh2_Mul_noUNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding, number_of_cols=number_of_cols).to(device)
