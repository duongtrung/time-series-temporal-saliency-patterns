...
input_window = 480           # input_window = previous time steps - output_window
output_window = 336
start_lr = 0.000001
epochs = 25
...
train_data, val_data, test_data = get_ETTh2_data(dataset=dataset, target=target, input_window=input_window, output_window=output_window, device=device)

if with_Unet == True:
    model = Transformer_ETTh2_UNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding).to(device)
else:
    model = Transformer_ETTh2_noUNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding).to(device)
