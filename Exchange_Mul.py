...
data = "Exchange-AF"                # Options: "ETT", "Electricity", "weather" 
dataset = "exchange_rate.csv"       # Options: "ETTh1.csv", "ETTh2.csv", "ETTm1.csv", "ECL.csv", "WTH.csv" 
target = "OT"               # Options: "OT" for {ETTh1.csv, ETTh2.csv, ETTm1.csv}, "MT_320" for ECL.csv, "WetBulbCelsius" for weather
number_of_cols = 8          # Options: 7 for {ETTh1.csv, ETTh2.csv, ETTm1.csv}, 8 for exchange_rate.csv 321 for ECL.csv, 12 for WTH.csv

input_window = 790           # input_window = previous time steps - output_window
output_window = 720
start_lr = 0.000001
epochs = 30
...
train_data, val_data, test_data = get_Exchange_AF_mul(dataset=dataset, target=target, input_window=input_window, output_window=output_window, device=device, number_of_cols=number_of_cols)

if with_Unet == True:
    model = v5_Exchange_Mul_UNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding, number_of_cols=number_of_cols).to(device)
else:
    model = v5_Exchange_Mul_noUNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding, number_of_cols=number_of_cols).to(device)
