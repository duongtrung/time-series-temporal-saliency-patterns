...
data = "ETTm1-AF"                # Options: "ETT", "Electricity", "weather", "ETT-N-HITS"
dataset = "ETTm1.csv"       # Options: "ETTh1.csv", "ETTh2.csv", "ETTm1.csv", "ECL.csv", "WTH.csv" 
...
input_window = 408           # input_window = previous time steps - output_window
output_window = 288
start_lr = 0.00001
epochs = 20
...
nhead=8
dropout=0.2
max_len_positional_encoding = 7000
...
train_data, val_data, test_data = get_ETTm1_AF_mul(dataset=dataset, target=target, input_window=input_window, output_window=output_window, device=device, number_of_cols=number_of_cols)

if with_Unet == True: 
    model = v5_ETTm1_Mul_UNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding, number_of_cols=number_of_cols).to(device)
else:
    model = v5_ETTm1_Mul_noUNet(nhidden=nhidden, num_layers=num_layers, nhead=nhead, dropout=dropout, in_channels=input_window, out_channels=output_window, max_len_positional_encoding=max_len_positional_encoding, number_of_cols=number_of_cols).to(device)
